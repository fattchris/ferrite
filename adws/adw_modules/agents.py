...
Hermes port: uses agent_hermes (OpenRouter API + Claude Code/Codex CLI)
instead of agent_pi.
"""
...
from . import agent_hermes, permissions, prompts
from .data_types import (AgentCall, AgentConfig, EnvelopeBase, EventRecord,
                         GateCheck, GateReport, Phase, HermesRequest,
...
def validate(cfg: SSSFConfig, required: list[str]) -> None:
    ...
    problems = []
    valid_backends = {"openrouter", "litellm", "pi", "claude_code", "codex"}
    for name in required:
        try:
            ...
            continue
        if agent.coding_agent not in valid_backends:
            problems.append(f"agent {name!r}: coding_agent {agent.coding_agent!r} "
                            f"is not a valid Hermes backend (one of: {valid_backends})")
        ...
        try:
            agent_hermes.resolve_model(agent.model)
...
def execute(run, phase: Phase, call: AgentCall) -> EnvelopeBase:
    """One agent call: render prompts -> hermes run -> typed parse -> gates -> envelope."""
    ...
    latest: agent_hermes.HermesResult | None = None
    spent = UsageBreakdown()

    def send(prompt_text: str) -> agent_hermes.HermesResult:
        nonlocal latest
        request = HermesRequest(
            ...
            coding_agent=agent.coding_agent,
            session_id=session_id,
            session_dir=str((agent_dir / "sessions").resolve()),
            raw_output_path=str((agent_dir / "raw_output.jsonl").resolve()),
            tools=agent.tools,
            extensions=agent.harness_engineering,
            cwd=str(run.repo_root),
        )
        result = agent_hermes.run(
            request,
...
def _event_forwarder(run, phase: Phase, agent_name: str):
    ...
    tracker = agent_hermes.ToolCallTracker()
