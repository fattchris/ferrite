...
class AgentConfig(BaseModel):
    name: str
    coding_agent: Literal["openrouter", "litellm", "pi", "claude_code", "codex"] = "openrouter"
...
# Replaces PiRequest/PiResult. The Hermes backend supports three coding_agent
# modes: "openrouter" (LLM API), "claude_code" (Claude CLI), "codex" (Codex CLI).

class HermesRequest(BaseModel):
    """Everything one non-interactive agent run needs."""

    prompt: str
    system_prompt: str = ""
    model: str                      # provider/model-id (e.g. "zai/glm-5.2")
    thinking: str = "medium"        # off | minimal | low | medium | high | max
    coding_agent: str = "openrouter"  # "openrouter" | "claude_code" | "codex"
    session_id: str = ""            # for CLI agents that support session resume
    session_dir: str = ""           # CLI agent session storage
...
class UsageBreakdown(BaseModel):
    ...
    def add_turn(self, usage: dict, total_tokens: int) -> None:
        """Fold in one usage object from any backend."""
        cost = usage.get("cost") or {}
        self.input_tokens += usage.get("input") or usage.get("prompt_tokens") or 0
        self.output_tokens += usage.get("output") or usage.get("completion_tokens") or 0
        self.cache_read_tokens += usage.get("cacheRead") or usage.get("cache_read") or 0
        self.cache_write_tokens += usage.get("cacheWrite") or usage.get("cache_write") or 0
...
class HermesResult(BaseModel):
    ...
    context_window: int = 0         # 0 when the model's ceiling is unknown
