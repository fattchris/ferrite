...
- **Use `codegraph_explore` first** — query the knowledge graph for the structural
  map (callers, callees, dependencies, test coverage gaps) before reading any file.
  This gives you the codebase architecture in one call instead of crawling files
  one by one.
- After `codegraph_explore`, read specific files only when you need details the
  graph doesn't provide (function bodies, comment context, exact logic).
