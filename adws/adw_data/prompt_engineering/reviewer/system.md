...
- **Use `codegraph_explore` to check blast radius** — query the graph for each changed file to see what depends on it, what callers exist, and whether tests cover the changed symbols. This catches regressions that reading changed files alone misses.
